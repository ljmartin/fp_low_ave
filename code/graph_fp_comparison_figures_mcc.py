...
ser = after_cats_trim['mcc_cats'] - after_morgan_trim['mcc_morgan']
...
#plot AVE vs EF:
ax[0,1].scatter(after_morgan_trim['ave_morgan'], after_morgan_trim['mcc_morgan'], 
                c='C0', label='Morgan', zorder=0, alpha=0.85)
ax[0,1].scatter(after_cats_trim['ave_cats'], after_cats_trim['mcc_cats'], 
                c='C1', label='CATS',zorder=1, alpha=0.65)
ax[0,1].axvline(0, linestyle='--', c='k')
ax[0,1].set_ylabel('Matthews corr. coefficient (MCC)')
ax[0,1].legend()
...
fig.savefig('./processed_data/graph_fp_comparison/comparison_mcc.png')
