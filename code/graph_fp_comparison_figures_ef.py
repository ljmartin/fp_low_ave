...
#first remove inf and nan values coming from a zero in either score:
ser = np.log(after_cats_trim['ef_cats'] / after_morgan_trim['ef_morgan'])
ser = ser[~ser.isin([np.nan, np.inf, -np.inf])]
#then calculate the mean of relative EF:
tr = calc_hpd(ser)
...
#plot AVE vs EF:
ax[0,1].scatter(after_morgan_trim['ave_morgan'], after_morgan_trim['ef_morgan'], 
                c='C0', label='Morgan', zorder=0, alpha=0.85)
ax[0,1].scatter(after_cats_trim['ave_cats'], after_cats_trim['ef_cats'], 
                c='C1', label='CATS',zorder=1, alpha=0.65)
ax[0,1].axvline(0, linestyle='--', c='k')
ax[0,1].set_ylabel('Enrichment factor (EF)')
ax[0,1].legend()
...
ax[1,0].set_xlabel('$\dfrac{EF_{CATS}}{EF_{Morgan}}$')
...
ax[1,1].set_xlabel('$\dfrac{EF_{CATS}}{EF_{Morgan}}$')
...
fig.savefig('./processed_data/graph_fp_comparison/comparison_ef.png')
